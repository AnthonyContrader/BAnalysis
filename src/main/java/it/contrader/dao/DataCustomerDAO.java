...
public class DataCustomerDAO {
	...
	private final String QUERY_ALL = "SELECT * FROM datacustomer";
	private final String QUERY_CREATE = "INSERT INTO datacustomer (name, surname, age, email, phonenumber) VALUES (?,?,?,?,?)";
	private final String QUERY_READ = "SELECT * FROM datacustomer WHERE id=?";
	private final String QUERY_UPDATE = "UPDATE datacustomer SET name=?, surname=?, age=?, email=?, phonenumber=? WHERE id=?";
	private final String QUERY_DELETE = "DELETE FROM datacustomer WHERE id=?";
	
	public DataCustomerDAO()
	{}
	public List<DataCustomer> getAll() {
		List<DataCustomer> datacustomersList = new ArrayList<>();
		...
		try {
			...
			while (resultSet.next()) {
				...
				datacustomersList.add(datacustomer);
			}
	...
	public DataCustomer read(int i) {
		Connection connection = ConnectionSingleton.getInstance();
		try {
	...
	public boolean update(DataCustomer datacustomerToUpdate) {
		...
		// Controllare se l'id è presente
		...
		if (!datacustomerRead.equals(datacustomerToUpdate)) {
			try {
				// riempire gli datacustomerToUpdate oggetti
